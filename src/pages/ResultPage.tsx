...
const ResultPage: React.FC<ResultPageProps> = ({ navigate, params }) => {
  const total = Number(params?.total) || 0;
  const correct = Number(params?.correct) || 0;
  const wrongIds = (params?.wrong || '').split(',').filter(Boolean);
  ...
  const loadWrong = useCallback(async () => {
    ...
    try {
      // 在 Electron 中，我们使用本地文件系统读取题目
      const fs = require('fs');
      const path = require('path');
      const questionsPath = path.join(__dirname, '../../public/questions.json');
      
      if (fs.existsSync(questionsPath)) {
        const all: Question[] = JSON.parse(fs.readFileSync(questionsPath, 'utf8'));
